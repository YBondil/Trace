...
- Backend / Stockage : Architecture Client-Serveur "Maison" (Node.js). 
  - Serveur : Node.js (avec Express ou Fastify), hébergé gratuitement (ex: Render ou Railway).
  - Base de données : Hébergement distant (ex: PostgreSQL via Neon.tech ou MongoDB Atlas) pour stocker le texte et les métadonnées.
  - Stockage Fichiers : Bucket de stockage compatible S3 (ex: Cloudflare R2) pour héberger les photos.
  - Sécurité : Création d'un système d'authentification (ex: JWT) pour sécuriser l'accès à l'API et au journal personnel.
  
## Fonctionnalités Principales (MVP)
1. Saisie Journalière : Possibilité d'écrire un texte et d'attacher une photo pour le jour J.
...
3. Exploration :
   ...
   - Vue de lecture détaillée d'un jour passé quelconque.
